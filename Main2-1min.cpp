...
void TimeR(sf::Texture& T,sf::Sprite &S,sf::Clock &Clk){
	...
	RT.draw(rs);	
	rs.setSize(sf::Vector2f(126*(60-time)/120,5)); rs.setPosition(17,0);
	...
	S.setTexture(numT[(int)(60-time)/60/10],true);
	S.setPosition(0,12);
	RT.draw(S);
	...
	S.setTexture(numT[(int)(60-time)%10],true);
...
//MainM,DropM,ComboM,ClrM,HoldM,KOM,KOgetM
    ...
    while(window.isOpen()){
        ...
        window.clear();
     	...
     	if((start==0&&MainC.getElapsedTime().asSeconds()>60)||forceend){//////time's up
     		MainM.stop();
				...
				if(timeup==1&&MainC.getElapsedTime().asSeconds()>63){
					timeup=2;
