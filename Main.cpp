...
class PlayerInfo {
	public:
		...
		void KOreset(void) { //after being KO, reset
			...
			KOclk.restart();
			Opponent->KOget();
			if(Opponent->KO>=5) forceend=Playername+1;
			/*RField();
			Refresh();       */
		}
		void KOget() {
		...
		void setPos() {
			...
			switch(HoldB.size) {
				case 4:
					HoldB.blkS.setOrigin(46,34.5);
					break;
				case 3:
					HoldB.blkS.setOrigin(34.5,23);
					break;
				case 2:
					HoldB.blkS.setOrigin(23,23);
					break;
			}

			//NextB[0].blkS.setScale(1,1); NextB[0].blkS.;
			NextB[1].blkS.setScale(0.8,0.8);
			NextB[1].blkS.setPosition(406+484*Playername,240);
			NextB[2].blkS.setScale(0.75,0.75);
			NextB[2].blkS.setPosition(406+484*Playername,325);
			NextB[3].blkS.setScale(0.6,0.6);
			NextB[3].blkS.setPosition(406+484*Playername,394);
			NextB[4].blkS.setScale(0.5,0.5);
			NextB[4].blkS.setPosition(406+484*Playername,461);
			NextB[5].blkS.setScale(0.5,0.5);
			NextB[5].blkS.setPosition(406+484*Playername,524);
			HoldB.blkS.setScale(0.8,0.8);
			HoldB.blkS.setPosition(62+484*Playername,240);
		}
		void Hold() { /////////
			...
			swap(HoldB,NextB[0]);
			HoldB.reset(HoldB.Blocktype);
			NextB[0].reset(NextB[0].Blocktype);
			switch(HoldB.size) {
				case 4:
					HoldB.blkS.setOrigin(46,34.5);
					break;
		...
		*/
			...
			if(NextB[0].size==3) {
				...
				for(int j=0; j<5; j++) {
					...
					for(int i=0; i<4; i++) {
						...
						if(i==3) {
							if(j==0) SpecialTurn=0;
							else SpecialTurn=1;
							success=1;
						}
					}
					if(success) {
						NextB[0].posc.x+=KW3R[NextB[0].status][j].x;
						NextB[0].posc.y+=KW3R[NextB[0].status][j].y;
						break;
					}
				}
				if(!success) {
				...
				for(int j=0; j<5; j++) {
					...
					if(success) {
						NextB[0].posc.x+=KW4R[NextB[0].status][j].x;
						NextB[0].posc.y+=KW4R[NextB[0].status][j].y;
						break;
					}
				}
				if(!success) {
		...
		void Refresh() {
			int out=0;
			NextB[0].poscD.x=NextB[0].posc.x;
			NextB[0].poscD.y=NextB[0].posc.y;
			while(out==0) {
				NextB[0].poscD.y--;
				for(int i=0; i<4; i++) {
		...
		void Drop() {
			...
			if(Tspin&&NextB[0].Blocktype==BlockType::T) { //handle T-spin
				...
				int losttype=0;
				if(tX==9||Field[Fd(tY+1,tX+1)]!=0) Tspin++;
				else losttype=1;
				if(tX==0||Field[Fd(tY+1,tX-1)]!=0) Tspin++;
				else  losttype=2;
				if(tX==0||tY==0||Field[Fd(tY-1,tX-1)]!=0) Tspin++;
				else  losttype=3;
				if(tX==9||tY==0||Field[Fd(tY-1,tX+1)]!=0) Tspin++;
				else  losttype=4;
				if(Tspin>=3) {
					...
					switch(NextB[0].status) {
						case 0:
							if(losttype==3||losttype==4) Tspin=2;
							break;
						case 1:
							if(losttype==2||losttype==3) Tspin=2;
							break;
						case 2:
							if(losttype==1||losttype==2) Tspin=2;
							break;
						case 3:
							if(losttype==1||losttype==4) Tspin=2;
							break;
					}
				}
		...
		void RLine() { //refresh linesent
			...
			LineS.setOrigin(17*width,19);
			LineS.setScale(resize,resize);
		...
		void RTimeup2() {
			string s="win";
			if(KO>Opponent->KO)s="win";
			else if(KO<Opponent->KO) s="lose";
			else if(Linesent>Opponent->Linesent)s="win";
			else if(Linesent<Opponent->Linesent) s="lose";
			else if(Highest<Opponent->Highest)s="win";
			else if(Highest>Opponent->Highest) s="lose";
			if(forceend) {
...
void TimeR(sf::Texture& T,sf::Sprite &S,sf::Clock &Clk) {
	...
	rs.setSize(sf::Vector2f(126,5));
	rs.setPosition(17,0);
	rs.setFillColor(sf::Color(80,50,14));//gray
	RT.draw(rs);
	rs.setSize(sf::Vector2f(126*(120-time)/120,5));
	rs.setPosition(17,0);
...
int main() {
	...
	int start=1,timeup=0;////////////////start should =1
...
//MainM,DropM,ComboM,ClrM,HoldM,KOM,KOgetM
	...
	if(playernum==1)Player[1].reset(1,0,&Player[0]);
	else Player[1].reset(1,tmpbomb,&Player[0]);
	...
	while(window.isOpen()) {
		...
		if(start) { //start
			...
			if(start==1) {
				...
				if(window.hasFocus()) {
					start++;
					if(sf::Music::Stopped==MainM.getStatus()) MainM.play();
				} else MainM.stop();
			} else start++;
		...
		for(int j=0; j<playernum; j++) {
			...
			if(Player[j].KOed&&Player[j].KOclk.getElapsedTime().asSeconds()<1.) { //////KOed
				...
				Player[j].KOed=0;
				Player[j].FdallS.setColor(sf::Color::White);
				Player[j].RField();
				Player[j].dwnclk.restart();
			}///////
		}
		window.draw(TimeS);
		window.draw(Player[0].LineS);
