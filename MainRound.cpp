...
sf::Sprite RoundS; sf::Texture RoundT;//////////////test
int playernum=2,forceend=0,nowRound=0;
...
class PlayerInfo{
    public:
        ...
        sf::Clock movclk,Comboclk,KOclk,Barclk,PCclk, Totalclk;
...
*/
        ...
        void Drop(){
        	...
        	if(this->KOed) return;
        	this->Holdyet=0;
            this->dwnclk.restart(); Opponent->dwnclk.restart(); this->Totalclk.restart(); Opponent->Totalclk.restart(); 
            
            this->movclk.restart();
        	this->Lineclr=0;
        	RoundS.setPosition(114+484*nowRound,177);
        	if(nowRound) nowRound=0; else nowRound=1;
...
void TimeR(sf::Texture& T,sf::Sprite &S,sf::Clock &Clk){
	...
	RT.draw(rs);	
	rs.setSize(sf::Vector2f(126*(1800-time)/1800,5)); rs.setPosition(17,0);
	...
	S.setTexture(numT[(int)(1800-time)/60/10],true);
	S.setPosition(0,12);
	RT.draw(S);
	...
	S.setTexture(numT[(int)(1800-time)%10],true);
...
void RoundR(){
	RoundS.setColor(sf::Color::White);
   	...
   	RT.create(230,463);
   	RT.clear(sf::Color::Black);   	
   	RoundT=RT.getTexture();
   	RoundS.setTexture(RoundT,true);
	RoundS.setColor(sf::Color(128,128,128,128));
	RoundS.setPosition(114+484,177);	
...
//MainM,DropM,ComboM,ClrM,HoldM,KOM,KOgetM
	...
	if(playernum==1)Player[1].reset(1,0,&Player[0]); else Player[1].reset(1,tmpbomb,&Player[0]);
    ...
    while(window.isOpen()){
        ...
        window.clear();
     	...
     	if(start==0&&MainC.getElapsedTime().asSeconds()>60*30){//////time's up
                ...
                case sf::Event::KeyPressed:
                   ...
                   switch(event.key.code){
                       ...
                       case sf::Keyboard::X:
                           ...
                           //turn0
                           	if(nowRound==0) Player[0].turnR();
                           	break;
                       case sf::Keyboard::Z:
                           //turn 1;
    						if(nowRound==0) Player[0].turnL();
                  	        break;
                       case sf::Keyboard::C:
                       case sf::Keyboard::LShift:
                           	//hold
                           	if(nowRound==0)	Player[0].Hold();//build-in check this->Holdyet
                           	break;
    ...
    if(nowRound==0){
		...
		if(Player[0].dwnclk.getElapsedTime()>sf::milliseconds(500)){
        	Player[0].movD();
		}
		if(Player[0].Totalclk.getElapsedTime()>sf::milliseconds(30000)){
        	Player[0].Drop();
		}
	}
	else if(playernum==2&&nowRound==1){
        ...
        if(Player[1].dwnclk.getElapsedTime()>sf::milliseconds(500)){
        	Player[1].movD();
		}
		if(Player[1].Totalclk.getElapsedTime()>sf::milliseconds(30000)){
        	Player[1].Drop();
		}
	}
		TimeR(TimeT,TimeS,MainC);
