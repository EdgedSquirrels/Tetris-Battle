...
typedef long long ll;
    sf::Music MainM,DropM,ComboM,ClrM,HoldM,KOM,KOgetM,TimeM;//MainM includes StartM, TimeM, win lose
...
class BlockInfo {
    public:
        ...
        void reset(int a,int sta=0,int cenx=5,int ceny=19){
        	this->Blocktype=a;
            this->posc.x=cenx;
            this->posc.y=ceny;
            this->status=sta;
...
class PlayerInfo{
    public:
        ...
        int KO=0,forcelose=0,PC=0;//PC:Perfectclaer
        ...
        int oBombRB=0;//red bar waiting to send
        int oBombOB=0;//orange bar waiting to send
        
        int oKO=0;//oppenent KO get (but haven't sent)
        int Highest=0;
   ...
   void KOreset(void){//after being KO, reset
  	 	KOed=1;
	   Highest-=Bomb;
	...
	void getBomb(int a){//get bombs from the opponent
		BombRB+=BombOB;
		BombOB=a; Barclk.restart();
	}
	void addBomb(int a){//add bombs to the field
		if(Bomb+a>20) a=20-Bomb;
		memmove(Field+a*10,Field,sizeof(char)*200);
	...
	void reset(int a,int temp,PlayerInfo* opp){// temp: initial bombs
		...
		KO=0; KOed=0;
        Linesent=0;
        Field=(char*)calloc(400,sizeof(char));
        ...
        BombRB=0;//red bar
        BombOB=0;//orange bar
        oBombRB=0;//red bar waiting to send
        oBombOB=0;//orange bar waiting to send
...
*/
        ...
        void Drop(){
			...
			if(Tspin&&NextB[0].Blocktype==BlockType::T){//handle T-spin
				...
				int losttype=0;
				if(tX==9||Field[Fd(tY+1,tX+1)]!=0) {Tspin++; losttype=1;}
				if(tX==0||Field[Fd(tY+1,tX-1)]!=0) {Tspin++; losttype=2;}
				if(tX==0||tY==0||Field[Fd(tY-1,tX-1)]!=0) {Tspin++; losttype=3;}
				if(tX==9||tY==0||Field[Fd(tY-1,tX+1)]!=0) {Tspin++; losttype=4;}
				if(Tspin>=3) {
					Tspin=1;
					if(Tspin==4){
						Tspin=2;
...
*/
			...
			if(Lineclr>0||Bombclr>0){
				...
				if(Lineclr>=4||Tspin){ //B2B
					if(B2B){
						...
						switch(Lineclr){
						case 1: if(Tspin==1)Linesent+=1; if(Tspin==2) point+=2;break;
				...
				switch(Lineclr){//print				
						case 1:
							switch(Tspin){
								case 0: case 2:printf("Single "); break;
				...
				if(Combo>=1){
					...
					if(Combo<=7)ComboM.openFromFile("Combo/"+to_string(Combo)+".ogg");
					//if(Playername==0){
					ComboM.stop();
					ComboM.setVolume(100);
					ComboM.play();
					//}
				}
				Linesent+=point;
				printf(" +%d\n",point);
				if(point&&playernum==2){
					oBombRB+=oBombOB;
					oBombOB=point;
				}
			} else Combo=-1;
			if(BombOB){
				if(point>BombOB) BombOB=0;
				else BombOB-=point;
				BombRB+=BombOB; BombOB=0;
			}
			if(BombRB){
				addBomb(BombRB); BombRB=0;
			}
			RField();
			RLine();
			Next();
		...
		void RBar(){
    		...
    		sf::RectangleShape R;
    		sf::RectangleShape O;
    		R.setSize(sf::Vector2f(8,23*BombRB));
    		O.setSize(sf::Vector2f(8,23*BombOB*per));
    		R.setFillColor(sf::Color(197,32,21));
    		O.setFillColor(sf::Color(214,135,30));
    		R.setPosition(0,463-23*BombRB);
    		O.setPosition(0,463-23*(BombRB+BombOB*per));
    		RT.draw(O); RT.draw(R);
    		RT.display();
    		BarT=RT.getTexture();
...
sf::IpAddress ip;
int timeup=0,Combobuffer=0/*record player[1]'s combo to ensure whether to restart the clock'*/,Blockbuffer[8]={0}/*7:Hold block*/;unsigned short port=38654; mutex glock;
PlayerInfo Player[2];


void Send(){
	while(timeup<1&&forceend==0){
		sf::Packet packet;
		glock.lock();
		packet<<Player[0].forcelose<<Player[0].KOed <<Player[0].Linesent <<Player[0].Combo <<Player[0].BombRB <<Player[0].BombOB 
		<<Player[0].oBombRB <<Player[0].oBombOB <<Player[0].oKO <<Player[0].Highest<<Player[0].HoldB.Blocktype;
		packet<<Player[0].NextB[0].Blocktype<<Player[0].NextB[0].status<<Player[0].NextB[0].posc.x<<Player[0].NextB[0].posc.x;
		
		for(int i=1;i<7;i++){
			packet<<Player[0].NextB[i].Blocktype;
		}		
		 //if(gsocket.send(Player[0].Field,200)!=sf::Socket::Done) cout<<"send error";
		Player[0].oBombRB=Player[0].oBombOB=Player[0].oKO=0;
		glock.unlock();
		//if(gsocket.send(packet)!=sf::Socket::Done) cout<<"send error ";
...
void Receive(){
	while(timeup<2&&forceend==0){
		sf::Packet packet;		
		packet.clear();
		//if(gsocket.receive(packet)!=sf::Socket::Done) cout<<"receive error ";
		
		glock.lock();/////////////////////////
		
		/*packet>>Player[1].forcelose>>Player[1].KOed >>Player[1].Linesent >>Player[1].Combo >>Player[1].BombRB >>Player[1].BombOB 
		>>Player[1].oBombRB >>Player[1].oBombOB >>Player[1].oKO >>Player[1].Highest>>Player[1].HoldB.Blocktype;
		packet>>Player[1].NextB[0].Blocktype>>Player[1].NextB[0].status>>Player[1].NextB[0].posc.x>>Player[1].NextB[0].posc.y;*/
		int YNsetpos=0;
		for(int i=1;i<7;i++){
		//	packet>>Player[1].NextB[i].Blocktype;
			if(Player[1].NextB[i].Blocktype!=Blockbuffer[i]){
				Player[1].NextB[i].reset(Player[1].NextB[i].Blocktype);YNsetpos=1;
			}
		}
		if(Player[1].HoldB.Blocktype!=Blockbuffer[7]){
			Player[1].HoldB.reset(Player[1].HoldB.Blocktype);YNsetpos=1;
		}
		if(YNsetpos) Player[1].setPos(); size_t received=0;
		//if(gsocket.receive(Player[1].Field,200,received)!=sf::Socket::Done) cout<<"receive error";
		for(int i=0;i<30;i++){
			printf("%d ",Player[1].Field[i]);
		} printf("\n");
		if(Player[1].oKO){
			Player[0].KOget(Player[1].oKO);
			Player[1].oKO=0;			
		}
		if(Player[1].oBombOB||Player[1].oBombRB){
			Player[0].getBomb(Player[1].oBombOB);
			Player[0].BombRB+=Player[1].oBombRB;
			Player[1].oBombOB=Player[1].oBombRB=0;
		}
		if(Player[1].forcelose) forceend=2;
		Player[1].NextB[0].reset(Player[1].NextB[0].Blocktype,Player[1].NextB[0].status,Player[1].NextB[0].posc.x,Player[1].NextB[0].posc.y);
		Player[1].RLine();
		Player[1].RField();
...
int setServer(){
	if(glistener.listen(port)!=sf::Socket::Done) {
		cout<<"Binding error "; return 1;
	} else cout<<"Binding successfully!\n";
	if(glistener.accept(gsocket)!=sf::Socket::Done){
		cout<<"Connection error "; return 1;
	}  cout<<"Connected succesfully!\n"; return 0;	
}
int setClient(){
	do{
		printf("Q: What's your server's IP address?\nA: ");
 	   cin>>ip; 
 	   if(ip==sf::IpAddress::None){
 	   		printf("Invalid input!!!\n"); continue;
		}
	}while(false);
	if(gsocket.connect(ip,port)!=sf::Socket::Done) {
		cout<<"Connection error "; return 1;
	}
...
//MainM,DropM,ComboM,ClrM,HoldM,KOM,KOgetM
    ...
    if(!ComboM.openFromFile("Combo/1.ogg")) return EXIT_FAILURE;
	...
	Player[0].reset(0,0,&Player[1]); 
	Player[1].reset(1,0,&Player[0]);
	...
	char cc='0';
    do{
    	printf("Q: you want to be a server<s> or a client<c>?\nA: ");
 	   cin>>cc; 
 	   switch(cc){
 	   		case 'S':
 	   		case 's':
 	   			cc='s';if(setServer()) return 1; break;
 	   		case 'C':
 	   		case 'c':
 	   			cc='c';if(setClient()) return 1; break;
 	   		default:
 	   			continue;
		}
	}while(false);
	...
	if(cc=='s')window.setTitle("Tetris Test-Server"); else window.setTitle("Tetris Test-Client");
		
	thread Thread2(Send);
	thread Thread1(Receive);
    ...
    while(window.isOpen()){
        ...
        window.clear();
     	...
     	if(start==0&&MainC.getElapsedTime().asSeconds()>120){//////time's up
		...
		for(int j=0;j<playernum;j++){
			...
			if(Player[j].BombOB+Player[j].BombOB){
				Player[j].RBar();
